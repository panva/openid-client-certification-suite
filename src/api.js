...
class API {
  ...
  async createTestPlan({ planName, configuration, variant } = {}) {
    ...
    //_
    assert.equal(statusCode, 201, JSON.stringify(response))
  ...
  async getTestExposed({ moduleId } = {}) {
    assert(moduleId, 'argument property "moduleId" missing')

    const { statusCode, body: response } = await this.get(`api/runner/${moduleId}`)
  ...
  async waitForState({
    ...
    throw new Error(
      `Timed out waiting for test module ${moduleId} to be in one of states: ${[...states].join(
        ', ',
      )}`,
    )
  }
